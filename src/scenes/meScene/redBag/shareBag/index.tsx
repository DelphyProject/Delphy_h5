...
import './index.less';
interface ShareBagProps {
  serverData: any;
  marketPageState: any;
}
interface ShareBagState {
  height: string;
  isShow: boolean;
  phoneNumber: string;
  timeCount: number;
...
type Props = ShareBagProps & DispatchProp & RouteComponentProps;
class ShareBag extends React.Component<Props, ShareBagState> {
  time: any;
  constructor(props) {
    super(props);
    this.state = {
  ...
  getCheckCode = () => {
    this.props.dispatch(
      //@ts-ignore
      fetchAction.fetchGetShareCheckCode(this.state.phoneNumber, null, val => {
        if (val.code == 200) {
          this.setState({
            // timeState: false, vertifyImgFlag: false
          });
        } else {
          showToast(val.msg, 3);
        }
  ...
  register = () => {
    ...
    params.inviteCode = window.sessionStorage.getItem('invitationCode') || '';
    ...
    this.props.dispatch(
      ...
      fetchAction.fetchRegisterUser(params, ret => {
        ...
        if (ret.code == 200) {
          ...
          sessionStorage.setItem('rewardAmount', '20');
          showToast('恭喜您注册成功！', 2);
          setTimeout(() => {
  ...
  render() {
        ...
        return (
          <div
            ...
            <div className="input-box">
              <p className="alert-title">注册领取20预测币</p>
    ...
    return (
      <div className="share-bag-warp" style={{ minHeight: this.state.height }}>
        ...
        <div className="up-warp">
          ...
          <p>1.每个被邀请人注册天算后，即可免费获得20预测币，可以参与天算预测，获取更多天算DPY。</p>
          <p>2.被邀请人注册后，若在天算中预测获胜3场，您可再获得1DPY+20预测币。</p>
          <p>3.若发现有任何不正当行为，例如造假、刷量、误导用户等，一经发现，将取消其相关奖励。</p>
          <p>4.本功能最终解释权归天算基金会所有。</p>
        </div>
...
export default connect(mapStateToProps)(withRouter(ShareBag));
